...
public class Rule {
    ...
    private int computeSumOfLiveNeighbors( Cell cell, Cell[][] world ) {
        ...
        for ( int[] neighbor : neighbors ) {
            ...
            if ( !( isVisibleNeighborPosition( neighborRowPosition, neighborColumnPosition, worldWith, worldHeight ) ) ) {
            ...
            if ( neighborCell.getCellStatus() == Cell.CellStatus.ALIVE )
                sumOfLiveOfNeighbors++;
        }
    ...
    private int fixPosition( int cellPosition, int worldBound ) {
        if ( cellPosition >= worldBound )
            return cellPosition - worldBound;
    ...
    private boolean isVisibleNeighborPosition( int neighborRowPosition, int neighborColumnPosition, int worldWith, int worldHeight ) {
        return (neighborRowPosition < worldHeight && neighborRowPosition >= 0) && (neighborColumnPosition < worldWith && neighborColumnPosition >= 0) ;
    }

    private void setCellStatus( Cell cell, Cell[][] world, int sumOfLiveNeighbors ) {
        Cell.CellStatus cellStatus = world[cell.getRowPosition()][cell.getColumnPosition()].getCellStatus();
