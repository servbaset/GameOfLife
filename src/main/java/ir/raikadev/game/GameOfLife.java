package ir.raikadev.game;

import ir.raikadev.cell.Cell;
import ir.raikadev.rule.Rule;
...
public class GameOfLife {
    ...
    for (int row = 0; row < world.length; row++) {
      for (int column = 0; column < world[row].length; column++) {
        Cell.CellStatus cellStatus =
            random.nextInt(100) < 20 ? Cell.CellStatus.ALIVE : Cell.CellStatus.DEAD;
  ...
  private void showWorld() {
    for (Cell[] cells : world) {
      for (Cell cell : cells) System.out.print(cell);
      System.out.println();
    }
